...
class TestBuildAnalyzer:
    ...
    def test_combine_sources(self):
        ...
        build1 = BuildDetails(
            name="Multi Build Corruption",
            source_url="https://example.com/multi1",
            items=[item]
        )

        build2 = BuildDetails(
            name="Multi Build Bossing",
            source_url="https://example.com/multi2",
            items=[item]
        )

        source_mapping = {
            build1.source_url: "corruption",
            build2.source_url: "bossing"
        }

        result = analyzer.analyze([build1, build2], source_mapping)

        candidate = result.exalted_candidates[0]
        # Should have both sources since different URLs
        assert "corruption" in candidate.sources
        assert "bossing" in candidate.sources
    ...
    def test_semantic_priorities(self):
        ...
        idol = IdolDTO(name="Minor Idol", size="Minor Idol (1x1)", modifiers=["Mod"])
    ...
    def test_estimated_rule_count(self):
        ...
        idol = IdolDTO(name="Minor Idol", size="Minor", modifiers=["M"])
