...
class TestRuleBuilderBasic:
    ...
    def test_idol_candidate_conversion(self):
        candidate = IdolCandidate(
            size='Grand', modifiers=frozenset(['MA', 'MB']),
...
class TestRuleBuilderAffixPreservation:
    ...
    def test_multiple_idol_modifiers_stay_together(self):
        candidate = IdolCandidate(
            size='Small', modifiers=frozenset(['M1', 'M2', 'M3']),
...
class TestRuleBuilderDataPreservation:
    def test_semantic_priority_preserved(self):
        e = ExaltedCandidate(base_key=('W', 1, 1), affixes=frozenset([('D', 5)]), score=100.0, semantic_priority=100)
        i = IdolCandidate(size='L', modifiers=frozenset(['C']), score=80.0, semantic_priority=70)
    ...
    def test_analyzer_score_preserved_exactly(self):
        candidate = ExaltedCandidate(
            base_key=('Item', 1, 1), affixes=frozenset([('Test', 3)]),
    ...
    def test_statistics_preserved(self):
        candidate = ExaltedCandidate(
            base_key=('Test', 1, 1), affixes=frozenset([('Stat', 2)]),
    ...
    def test_sources_preserved_and_copied(self):
        ...
        candidate = ExaltedCandidate(
            base_key=('Test', 1, 1), affixes=frozenset([('A', 1)]),
...
class TestRuleBuilderOrdering:
    def test_deterministic_ordering(self):
        e1 = ExaltedCandidate(base_key=('S1', 1, 1), affixes=frozenset([('A', 1)]), score=90.0, semantic_priority=100)
        e2 = ExaltedCandidate(base_key=('S2', 2, 2), affixes=frozenset([('B', 2)]), score=95.0, semantic_priority=100)
    ...
    def test_same_input_produces_identical_output(self):
        candidates = [
            ExaltedCandidate(base_key=(f'Slot{i}', i, i), affixes=frozenset([(f'Aff{i}', i)]), score=100-i, semantic_priority=100)
            for i in range(5)
        ]
...
class TestRuleBuilderCounts:
    def test_correct_category_counts(self):
        analysis = AnalysisResult(
            exalted_candidates=[
                ExaltedCandidate(base_key=('A', 1, 1), affixes=frozenset([('X', 1)]), score=10, semantic_priority=100),
                ExaltedCandidate(base_key=('B', 2, 2), affixes=frozenset([('Y', 2)]), score=20, semantic_priority=100)
            ],
            idol_candidates=[IdolCandidate(size='S1', modifiers=frozenset(['M']), score=30, semantic_priority=70)],
    ...
    def test_correct_total_rule_count(self):
        analysis = AnalysisResult(
            exalted_candidates=[ExaltedCandidate(base_key=(f'A{i}', i, i), affixes=frozenset([('X', 1)]), score=10, semantic_priority=100) for i in range(10)],
            idol_candidates=[IdolCandidate(size=f'S{i}', modifiers=frozenset(['M']), score=20, semantic_priority=70) for i in range(5)]
        )
        result = RuleBuilder().build(analysis)
...
class TestRuleBuilder140Limit:
    def test_over_140_rules_are_not_deleted(self):
        candidates = [
            ExaltedCandidate(base_key=(f'Slot{i}', i%10, i%5), affixes=frozenset([(f'A{i}', 1)]), score=100-i, semantic_priority=100)
            for i in range(150)
        ]
    ...
    def test_over_140_produces_exceeds_limit_warning(self):
        candidates = [
            ExaltedCandidate(base_key=(f'S{i}', i%10, i%5), affixes=frozenset([('A', 1)]), score=10, semantic_priority=100)
            for i in range(145)
        ]
        result = RuleBuilder().build(AnalysisResult(exalted_candidates=candidates))
